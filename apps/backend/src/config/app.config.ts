...
export function getAppConfig() {
  ...
  /** Cloud Run sets K_SERVICE; HTTPS-only — SameSite=None cookies must use Secure for cross-origin SPAs. */
  const deviceCookieSecure =
    nodeEnv === 'production' ||
    process.env.DEVICE_COOKIE_SECURE === 'true' ||
    isCloudRun;
  return {
    nodeEnv,
    ...
    /** Set Secure flag on device cookies (required on HTTPS; auto-on in production or Cloud Run). */
    deviceCookieSecure,
